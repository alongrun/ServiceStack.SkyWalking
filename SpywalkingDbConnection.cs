...
{
    public class SpywalkingDbConnection : DbConnection
    {
        private TimeSpan timerTimeSpan;
        private bool wasPreviouslyUsed;

        public SpywalkingDbConnection(DbConnection connection)
            : this(connection, DbProviderFactories.GetFactory(connection))
        { 
        }

        public SpywalkingDbConnection(DbConnection connection, DbProviderFactory providerFactory)
            : this(connection, providerFactory, Guid.NewGuid())
        { 
        }

        public SpywalkingDbConnection(DbConnection connection, DbProviderFactory providerFactory, Guid connectionId)
        {
        ...
        {
            return new SpywalkingDbTransaction(InnerConnection.BeginTransaction(isolationLevel), this);
        }
